...
| ~~`scripts/flux_profiler.sh`~~ (removed) | Legacy shell profile applier | C — **retired** | Was adapted from Encore `encore_profiler` | n/a (removed) | **Done** — deleted in the Stage 2 Increment 5 legacy removal, after the V2 execution cutover made it unreachable. **Its device knowledge was not discarded and is not unattributed:** the per-SoC paths and values it carried live on in `jni/device/*` as declarative data, which retains the dual copyright and the Encore attribution in its own right (see the `jni/device/*` row). Deleting the implementation does not retire the obligation the retained knowledge carries | n/a (not built, not shipped) |
| `scripts/flux_utility.sh` | Utility shell entry point (diagnostics/maintenance; **not** an apply path) | C | Adapted from Encore `encore_utility` | Dual header; NOTICE §1 | Pending | Ships under Apache-2.0 with attribution |
| ~~`jni/Profiler.cpp`, `jni/Profiler.hpp`~~ (removed) | Legacy profile dispatcher (`system("flux_profiler ...")`) and telemetry-provider seam | C — **retired** | Was adapted from Encore's profiler dispatch | n/a (removed) | **Done** — deleted in Stage 2 Increment 5. The dispatchers went in the cutover (Increment 4); what remained was a telemetry provider that was assigned and never read. Replaced by `flux::execution::ExecutionRuntime` (Category A) | n/a (not built, not shipped) |
