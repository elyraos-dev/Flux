...
bool GameRegistry::load_from_json(const std::string &filename) {
    ...
    std::vector<FluxGameList> new_list;
...
bool GameRegistry::populate_from_base(const std::string &gamelist, const std::string &baselist) {
    ...
    std::vector<FluxGameList> game_list;
...
void GameRegistry::update_gamelist(const std::vector<FluxGameList> &new_list) {
...
std::optional<FluxGameList> GameRegistry::find_game(const std::string &package_name) const {
...
const FluxGameList *GameRegistry::find_game_ptr(const std::string &package_name) const {
    std::shared_lock lock(mutex_);
    auto it = game_packages_.find(package_name);
    if (it != game_packages_.end()) {
        return &it->second;
...
bool GameRegistry::validate_game_entry(const FluxGameList &game) const {
    if (game.package_name.empty()) {
