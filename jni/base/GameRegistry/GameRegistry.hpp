...
class GameRegistry {
private:
    std::unordered_map<std::string, FluxGameList> game_packages_;
...
public:
     ...
     */
    void update_gamelist(const std::vector<FluxGameList> &new_list);
     ...
     */
    std::optional<FluxGameList> find_game(const std::string &package_name) const;
     ...
     */
    const FluxGameList *find_game_ptr(const std::string &package_name) const;
...
private:
     ...
     */
    bool validate_game_entry(const FluxGameList &game) const;
};
